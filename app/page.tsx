  ...
  return (
    <div className="relative w-screen h-screen bg-black overflow-hidden">
      ...
      <button
        ...
        className={`absolute top-5 z-20 text-white p-2 bg-black bg-opacity-50 hover:bg-opacity-70 rounded-md border border-white/30 hover:border-white/60 transition-all duration-500 group
                    ${showInfo ? 'left-[calc(5rem+420px)]' : 'left-5'}`}
        title={showInfo ? "Hide Info" : "Show Info"}
      >
        <svg 
          className={`w-4 h-4 transition-transform duration-300 ${showInfo ? 'rotate-0' : 'rotate-180'}`}
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
      ...
      <div className={`absolute top-5 left-5 text-white z-10 p-4 bg-black rounded-lg font-mono
                       ...
                       ${showInfo ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full pointer-events-none'}`}>
        {epoch === 'planck' && ( 
